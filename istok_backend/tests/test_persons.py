...
Проверяет создание персон, автогенерацию событий жизни,
автоподстановку фамилии/отчества и систему песочницы.
"""
import pytest
...
async def test_create_person_and_auto_birth_event(client: AsyncClient):
    ...
    # 2. Отправляем POST запрос на создание
...
async def test_get_persons_list(client: AsyncClient):
    ...
    response = await client.get("/persons/?tree_id=1&status=confirmed")
...
async def test_auto_generate_patronymic(client: AsyncClient):
    ...
    Проверяет стандартные случаи и исключения (Никита -> Никитич).
    """
    # 1. Создаем отцов с разными именами
    # Иван (стандартное)
    res_f1 = await client.post("/persons/", json={
        "first_name": "Иван", "gender": "male", "tree_id": 1, "skip_duplicate_check": True
    })
    f1_id = res_f1.json()["id"]

    # Дмитрий (на 'й')
    res_f2 = await client.post("/persons/", json={
        "first_name": "Дмитрий", "gender": "male", "tree_id": 1, "skip_duplicate_check": True
    })
    f2_id = res_f2.json()["id"]

    # Никита (исключение)
    res_f3 = await client.post("/persons/", json={
        "first_name": "Никита", "gender": "male", "tree_id": 1, "skip_duplicate_check": True
    })
    ...
    assert res_son.json()["middle_name"] == "Иванович", \
        f"Ожидался Иванович, получено {res_son.json()['middle_name']}"
    ...
    assert res_daughter.json()["middle_name"] == "Дмитриевна", \
        f"Ожидалась Дмитриевна, получено {res_daughter.json()['middle_name']}"
    ...
    assert res_son_nikita.json()["middle_name"] == "Никитич", \
        f"Ожидался Никитич, получено {res_son_nikita.json()['middle_name']}"
...
async def test_patronymic_different_cultures(client: AsyncClient):
    ...
    """
    # Русский
    res_f_ru = await client.post("/persons/", json={
        "first_name": "Иван", "gender": "male", "tree_id": 1, "culture": "ru", "skip_duplicate_check": True
    })
    f_ru_id = res_f_ru.json()["id"]

    res_son_ru = await client.post("/persons/", json={
        "first_name": "Алексей", "gender": "male", "tree_id": 1,
        "father_id": f_ru_id, "culture": "ru", "skip_duplicate_check": True
    })
    assert res_son_ru.json()["middle_name"] == "Иванович", \
        f"Русское отчество: ожидался Иванович, получено {res_son_ru.json()['middle_name']}"

    # Украинский
    res_f_uk = await client.post("/persons/", json={
        "first_name": "Іван", "gender": "male", "tree_id": 1, "culture": "uk", "skip_duplicate_check": True
    })
    f_uk_id = res_f_uk.json()["id"]

    res_son_uk = await client.post("/persons/", json={
        "first_name": "Олексій", "gender": "male", "tree_id": 1,
        "father_id": f_uk_id, "culture": "uk", "skip_duplicate_check": True
    })
    assert res_son_uk.json()["middle_name"] == "Іванович", \
        f"Украинское отчество: ожидался Іванович, получено {res_son_uk.json()['middle_name']}"

    # Казахский
    res_f_kk = await client.post("/persons/", json={
        "first_name": "Асан", "gender": "male", "tree_id": 1, "culture": "kk", "skip_duplicate_check": True
    })
    f_kk_id = res_f_kk.json()["id"]

    res_son_kk = await client.post("/persons/", json={
        "first_name": "Болат", "gender": "male", "tree_id": 1,
        "father_id": f_kk_id, "culture": "kk", "skip_duplicate_check": True
    })
    assert "ұлы" in res_son_kk.json()["middle_name"], \
        f"Казахское отчество: ожидалось 'ұлы', получено {res_son_kk.json()['middle_name']}"

    # Шведский
    res_f_sv = await client.post("/persons/", json={
        "first_name": "Erik", "gender": "male", "tree_id": 1, "culture": "sv", "skip_duplicate_check": True
    })
    f_sv_id = res_f_sv.json()["id"]

    res_son_sv = await client.post("/persons/", json={
        "first_name": "Lars", "gender": "male", "tree_id": 1,
        "father_id": f_sv_id, "culture": "sv", "skip_duplicate_check": True
    })
    assert res_son_sv.json()["middle_name"] == "Erikson", \
        f"Шведское отчество: ожидался Erikson, получено {res_son_sv.json()['middle_name']}"

    # Исландский
    res_f_is = await client.post("/persons/", json={
        "first_name": "Jón", "gender": "male", "tree_id": 1, "culture": "is", "skip_duplicate_check": True
    })
    f_is_id = res_f_is.json()["id"]

    res_daughter_is = await client.post("/persons/", json={
        "first_name": "Björk", "gender": "female", "tree_id": 1,
        "father_id": f_is_id, "culture": "is", "skip_duplicate_check": True
    })
    assert res_daughter_is.json()["middle_name"] == "Jóndóttir", \
        f"Исландское отчество: ожидалась Jóndóttir, получено {res_daughter_is.json()['middle_name']}"


@pytest.mark.asyncio
async def test_sandbox_and_duplicate_detection(client: AsyncClient):
    """
    Тест проверяет систему песочницы и поиска дубликатов:
    ...
    """
    # 1. Создаем первую персону с уникальным именем и подтверждаем её
    person1_data = {
        "first_name": "ТестИван",
        "last_name": "ТестПетров",
        "birth_date": "1970-05-15",
        "birth_place": "ТестМосква",
        "gender": "male",
        "tree_id": 1,
    ...
    person2_data = {
        ...
        "gender": "male",
        "tree_id": 1
        # skip_duplicate_check НЕ указан — проверяем поиск дубликатов
    }
    ...
    assert res2.json()["status"] == "sandbox", \
        f"Вторая персона должна быть в песочнице, статус: {res2.json()['status']}"

    # Проверяем, что нашли дубликат
    duplicates = res2.json().get("potential_duplicates", [])
    assert len(duplicates) > 0, "Дубликат не найден!"

    # Проверяем, что среди дубликатов есть первая персона (по ID)
    duplicate_ids = [d["id"] for d in duplicates]
    assert person1_id in duplicate_ids, \
        f"Первая персона (ID={person1_id}) не найдена среди дубликатов! Найдены: {duplicate_ids}"
    ...
    assert person1_id in confirmed_ids, "Первая персона должна быть в списке confirmed"
    assert person2_id in confirmed_ids, "Вторая персона должна быть в списке confirmed"
    assert person3_id not in confirmed_ids, "Слитая персона не должна быть в списке confirmed"